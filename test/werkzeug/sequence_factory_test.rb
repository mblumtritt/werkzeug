...
class SequenceFactoryTest < Test
  ...
  def test_linear_pong_int
    expectation = [4, 3, 2, 1, 2, 3, 4, 3, 2, 1]
    subject = Werkzeug::SequenceFactory.linear.ping_pong(4, 1, 1)
    assert_equal(expectation, Array.new(10, &subject))
  end

  def test_linear_pong_float
    expectation = [2, 1.75, 1.5, 1.25, 1.0, 1.25, 1.5, 1.75, 2.0, 1.75]
    subject = Werkzeug::SequenceFactory.linear.ping_pong(2, 1, 0.25)
    assert_equal(expectation, Array.new(10, &subject))
  end
